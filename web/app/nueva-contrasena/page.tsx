    ...
    return (
      <div className="min-h-dvh bg-background flex flex-col items-center justify-center gap-4 px-6">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          <h2 className="text-xl font-bold text-foreground">¡Contraseña creada!</h2>
    ...
    <div className="min-h-dvh bg-background flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm">
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-4 text-5xl shadow-md"
            style={{ background: "#F2C5CE" }}>
            🔑
          </div>
          <h1 className="text-2xl font-extrabold text-foreground">Crea tu contraseña</h1>
          <p className="text-muted-foreground mt-1.5 text-sm font-medium">
            La necesitarás cada vez que quieras entrar a la app
          </p>
        </motion.div>

        <motion.form initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.1 }} onSubmit={handleSubmit} className="flex flex-col gap-4">

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-wide">Contraseña</label>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-foreground uppercase tracking-wide">Confirmar contraseña</label>
            <div className="relative">
              <Lock size={17} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground" />
              <Input type={showPw ? "text" : "password"} placeholder="Repite la contraseña"
          ...
          <button type="submit" disabled={!password || !confirm || loading}
