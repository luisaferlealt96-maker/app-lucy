...
import {
  ArrowLeft, FlaskConical, Check, Calendar, Clock, MapPin,
  ExternalLink, FileText, Upload, Shield, X,
} from "lucide-react";
...
import { uploadDocumentoMedico } from "@/lib/supabase/storage";
import type { TipoAutorizacion } from "@/lib/supabase/types";
...
const TIPO_AUTH_MAP: Record<string, TipoAutorizacion> = {
...
function NuevoExamenContent() {
  ...
  const [form, setForm] = useState({
    ...
    nombre: "",
    tipo: "examen" as "laboratorio" | "examen" | "procedimiento",
    especialidad: "",
    lugar: "",
    fecha_solicitud: "",
    hora: "",
  ...
  const handleSave = async () => {
    ...
    if (tieneFecha && !form.fecha_solicitud) return;
    setSaving(true);
    const supabase = createClient();
    const { data: { user } } = await supabase.auth.getUser();
    const userId = user?.id ?? null;

    const { data: inserted } = await supabase.from("examenes").insert({
      paciente_id: form.paciente_id,
      nombre: form.nombre,
      tipo: form.tipo,
      especialidad: form.especialidad || null,
      lugar: form.lugar || null,
      fecha_solicitud: tieneFecha ? form.fecha_solicitud : null,
      hora: tieneFecha && form.hora ? form.hora : null,
    ...
    if (inserted?.id && archivoOrden) {
      const path = await uploadDocumentoMedico(archivoOrden, inserted.id, "orden");
      if (path) {
        await supabase.from("examenes").update({ archivo_orden_url: path }).eq("id", inserted.id);
      }
    }
    ...
    if (inserted?.id && necesitaAuth && authConfirmada && authForm.fecha_orden) {
      await supabase.from("autorizaciones_eps").insert({
        ...
        examen_id: inserted.id,
        created_by: userId,
      });
    }

    // Notificar por WhatsApp si tiene fecha, hora y lugar
    if (inserted?.id && tieneFecha && form.lugar && form.hora) {
      supabase.functions.invoke("notificar-citas", {
        body: { tipo: "nueva_cita", examen_id: inserted.id },
      }).catch(() => {});
    }
  ...
  const valido = form.paciente_id && form.nombre && !!form.especialidad &&
    (tieneFecha ? !!form.fecha_solicitud && !!form.lugar : true);

  const TIPOS = [
    { value: "laboratorio",   label: "🧪 Laboratorio",        desc: "Sangre, orina, biopsias..." },
    { value: "examen",        label: "🔬 Examen diagnóstico",  desc: "Rayos X, eco, resonancia..." },
    ...
    <div className="min-h-dvh bg-background pb-32 max-w-md md:max-w-xl mx-auto">
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <FlaskConical size={20} color="#C0546A" strokeWidth={2.5} />
          <div>
            <h1 className="text-xl font-extrabold text-foreground">Nuevo examen / laboratorio</h1>
      ...
      >
        ...
        <div className="rounded-2xl border border-border bg-card p-4 flex flex-col gap-3">
          <p className="text-sm font-bold text-foreground">¿Ya tiene fecha programada?</p>
          ...
          {!tieneFecha && (
            ...
            >
              El examen quedará registrado como <strong>pendiente por agendar</strong>. Puedes agregar la fecha cuando la tengas.
            </motion.p>
          )}
        </div>
            ...
            {TIPOS.map(t => (
              <button
                ...
                className={`rounded-xl p-3 text-left border-2 transition-all ${
                  form.tipo === t.value ? "border-primary bg-accent" : "border-border bg-card"
                }`}
              >
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-foreground uppercase tracking-wide">Nombre del examen / procedimiento</label>
          <Input
            placeholder="Ej: Hemograma, Ecografía abdominal, Radiografía de rodilla"
            value={form.nombre}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            <SelectTrigger className="rounded-xl border-border bg-card h-12">
              <SelectValue placeholder="Especialidad médica" />
            </SelectTrigger>
            <SelectContent>
        ...
        {/* Fecha + Ubicación (solo si tiene fecha programada) */}
        <AnimatePresence>
          {tieneFecha && (
            <motion.div
            ...
            >
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <DatePicker
                    ...
                    onChange={val => set("fecha_solicitud", val)}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    placeholder="Ej: Clínica del Country, laboratorio piso 2"
                    value={form.lugar}
                  ...
                  {form.lugar && (
                    <a
                      href={`https://maps.google.com/?q=${encodeURIComponent(form.lugar)}`}
                      target="_blank"
        ...
        {/* Orden médica */}
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-foreground uppercase tracking-wide">Orden médica en PDF (opcional)</label>
          <label className="flex items-center gap-3 rounded-xl border-2 border-dashed border-border bg-card px-4 py-3.5 cursor-pointer hover:border-primary transition-colors"
            style={archivoOrden ? { borderColor: "#9B8EC4", background: "#F5F3FF" } : {}}>
            <input type="file" accept=".pdf,application/pdf" className="hidden"
              onChange={e => setArchivoOrden(e.target.files?.[0] ?? null)} />
            {archivoOrden ? (
              <>
                <FileText size={18} color="#9B8EC4" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate" style={{ color: "#9B8EC4" }}>{archivoOrden.name}</p>
                  <p className="text-xs text-muted-foreground">{(archivoOrden.size / 1024).toFixed(0)} KB · PDF</p>
                </div>
                <button type="button" onClick={e => { e.preventDefault(); setArchivoOrden(null); }}
                  className="text-xs text-muted-foreground hover:text-foreground underline">Quitar</button>
              </>
            ) : (
              <>
                <Upload size={18} className="text-muted-foreground" />
                <div>
                  <p className="text-sm font-semibold text-foreground">Subir orden médica</p>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Select
            value={form.acompanante_id || "none"}
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Textarea
            placeholder={tieneFecha
              ? "Instrucciones, preparación, observaciones..."
              : "Por qué falta agendarlo, qué se necesita antes, instrucciones..."}
            value={form.notas}
                  ...
                  <div className="flex items-center justify-between rounded-xl px-3 py-2.5"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs font-semibold" style={{ color: "#2e7d32" }}>
                        Autorización lista — se crea junto con el examen
                      </p>
                    </div>
        ...
        >
          ...
          ) : saving ? "Guardando..." : tieneFecha ? (
            <><FlaskConical size={18} /> Guardar examen</>
          ) : (
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <DatePicker
                  ...
                  onChange={val => setAuth("fecha_orden", val)}
                  className="rounded-xl border-border bg-card"
                />
                {venc && (
              ...
              <div className={`overflow-hidden transition-all duration-300 -mx-1 px-1 ${authForm.estado === "en_tramite" ? "max-h-28 opacity-100" : "max-h-0 opacity-0 pointer-events-none"}`}>
                <div className="flex flex-col gap-1.5 pt-1">
                  ...
                  <DatePicker
                    ...
                    onChange={val => setAuth("fecha_envio_eps", val)}
                    className="rounded-xl border-border bg-card"
                  />
                </div>
              </div>
